	...
	for name, tc := range cases {
		for _, unadmittedWorkloadsObservabilityEnabled := range []bool{false, true} {
			t.Run(fmt.Sprintf("%s/gate_%t", name, unadmittedWorkloadsObservabilityEnabled), func(t *testing.T) {
				...
				assignment := flvAssigner.Assign(log, tc.counts)
...
func TestReclaimBeforePriorityPreemption(t *testing.T) {
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			...
			flvAssigner := New(wlInfo, testClusterQueue, resourceFlavors, false, &testOracle{tc.simulationResult}, nil, configapi.QuotaCheckBlockUndeclared, 0)
	...
	for name, tc := range cases {
		for _, enabled := range []bool{false, true} {
			t.Run(fmt.Sprintf("%s/gate_%t", name, enabled), func(t *testing.T) {
				...
				flvAssigner := New(wlInfo, clusterQueue, flavorMap, false, &testOracle{}, nil, configapi.QuotaCheckBlockUndeclared, 0)
...
func TestAssignFlavorsWithAllowedFlavors(t *testing.T) {
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			...
			assigner := New(wlInfo, cqSnapshot, resourceFlavors, false, &testOracle{}, nil, configapi.QuotaCheckBlockUndeclared, 0)
...
func TestIsNoFitDueToCapacityAndLimits(t *testing.T) {
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			...
			assigner := New(wlInfo, cqSnapshot, testFlavors, false, &testOracle{simulationResult: tc.simulationResult}, tc.replaceWl, configapi.QuotaCheckBlockUndeclared, 0)
			gotAssignment := assigner.Assign(log, nil)
...
func TestAssignFlavors_LeaderWorkerSetTASFlavor(t *testing.T) {
	...
	cases := map[string]struct {
		wlPods            []kueue.PodSet
		clusterQueue      kueue.ClusterQueue
		wantPodSetErrors  map[kueue.PodSetReference]string
		...
		"peers in the same group resolving to different TAS flavors are rejected": {
			...
			wantPodSetErrors: map[kueue.PodSetReference]string{
				"leader": "more than one TAS flavor assigned: tas-a, tas-b",
			},
			wantPodSetFlavors: map[kueue.PodSetReference]ResourceAssignment{
		...
		"leader in group where peer resolves to non-TAS flavor is rejected": {
			...
			// Flavors and podSetTopologyRequest returns "workload requires Topology, but
			// there is no TAS cache information" (checked before onlyTASFlavor).
			wlPods: []kueue.PodSet{
				*utiltestingapi.MakePodSet("leader", 1).
					RequiredTopologyRequest(corev1.LabelHostname).
					PodSetGroup("group1").
			...
			wantPodSetErrors: map[kueue.PodSetReference]string{
				"leader": "workload requires Topology, but there is no TAS cache information",
			},
			wantPodSetFlavors: map[kueue.PodSetReference]ResourceAssignment{
				"leader": {},
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			...
			flvAssigner := New(wlInfo, cq, resourceFlavors, false, &testOracle{}, nil, configapi.QuotaCheckBlockUndeclared, 0)
			assignment := flvAssigner.Assign(log, nil)

			gotErrors := map[kueue.PodSetReference]string{}
			gotFlavors := map[kueue.PodSetReference]ResourceAssignment{}
			for _, ps := range assignment.PodSets {
				if ps.Status.err != nil {
					gotErrors[ps.Name] = ps.Status.err.Error()
				}
				gotFlavors[ps.Name] = ps.Flavors
			}
			if diff := cmp.Diff(tc.wantPodSetErrors, gotErrors); diff != "" {
...
func TestWorkloadsTopologyRequests_RequiredTopologyRejectedForElasticWorkloadSlices(t *testing.T) {
	...
	if assignment.PodSets[0].Status.err == nil || assignment.PodSets[0].Status.err.Error() != "required topology is not supported with ElasticJobsViaWorkloadSlices" {
		t.Fatalf("expected leader podSet error to be %q, got %v", "required topology is not supported with ElasticJobsViaWorkloadSlices", assignment.PodSets[0].Status.err)
	}
}
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			...
			assigner := New(wlInfo, cqSnapshot, bookmarkTestFlavors(), false,
				...
				configapi.QuotaCheckBlockUndeclared, bookmarkTestCycle)
			assignment := assigner.Assign(log, nil)
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			...
			nominated := New(wlInfo, cqSnapshot, flavors, false, &testOracle{}, nil,
				configapi.QuotaCheckBlockUndeclared, bookmarkTestCycle).Assign(log, nil)
...
func TestElasticTASDoesNotDoubleCountReplacedSlice(t *testing.T) {
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			...
			a := New(next, cq, bookmarkTestFlavors(), false, &testOracle{}, oldInfo, configapi.QuotaCheckBlockUndeclared, bookmarkTestCycle).Assign(log, nil)
