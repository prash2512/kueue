...
func (a *FlavorAssigner) assignFlavors(ctx context.Context, log logr.Logger, counts []int32) Assignment {
	...
	for _, podSets := range groupedRequests.InOrder {
		...
		for resName, quantity := range requests.Iter() {
			...
			flavors, status, considered := a.findFlavorForPodSets(ctx, log, psIDs, requests, probeRequests, resName, assignment.Usage.Quota, assignedRequests)
			if probeRequests != nil && len(flavors) == 0 && !status.IsError() {
				...
				flavors, status, considered = a.findFlavorForPodSets(ctx, log, psIDs, requests, nil, resName, assignment.Usage.Quota, assignedRequests)
